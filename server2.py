...
class ProxyHTTPRequestHandler(BaseHTTPRequestHandler):
    ...
    def do_POST(self, body=True):
        ...
        try:
            ...
            if self.is_banner_method(post_body):
                self.send_error(503, 'Service Unavailable') 

            req_header = self.parse_headers()

            if b'Host' in self.headers:
                del self.headers[b'Host']
            self.headers[b'Host'] = hostname

            resp = requests.post(url, data=post_body, headers=self.headers, verify=False)
            sent = True
    ...
    def is_banner_method(self, body):
        try:
            d = json.loads(body)
            if 'method' in d:
                if d['method'] in BannedMethod:
                    return True
        except:
            return False
        return False
